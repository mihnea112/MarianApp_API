        ...
        let sql2=`INSERT INTO users (
               ...
               VALUES (
                ...
                if(warn){
                    ...
                    const token = jwt.sign({id: insertId, email, role:0}, process.env.TOKEN_KEY as string,
                        {
                        expiresIn: "2d",
                      });
...
app.get("/user", (req,res)=>{
    ...
    if(token != null)
    return res.status(201).json({role:(decoded as any).role,email:(decoded as any).email})
    else
...
app.get ("/userdata", async(req,res)=>{
    ...
    const decoded = jwt.verify(token, process.env.TOKEN_KEY as string) as object
    let sql =` SELECT name, email, adresa, telefon FROM users WHERE id = "${(decoded as any).id}"`;
        const resu = (await db.execute(sql))[0];
        console.log(resu);
